...
export async function waitForDeal(page) {
  await expect
...
export const color = (card) => (card.suit === "♦" || card.suit === "♥" ? "red" : "black");

// True when `lower` may sit directly beneath `upper` in a tableau run.
export const stacks = (upper, lower) =>
...
export function findSingleBottomCard(board) {
...
export function findTwoCardRunPlan(board) {
  const bottoms = board.map((col) => (col.length ? col[col.length - 1] : null));
  const headsSingle = (idx) => {
    const col = board[idx];
